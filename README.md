# 🚀 AyntraCore – Intelligent Support Engine
...
**AyntraCore** ist ein KI-gestütztes Support-System auf Basis von **Java 21** und **Spring Boot 3.2.x**.
Das System nutzt **RAG (Retrieval-Augmented Generation)**, um Support-Anfragen präzise, kontextbewusst und im Stil
definierbarer Persönlichkeiten (**Personas**) zu beantworten.
...
Aktuell ist AyntraCore in der **Phase 2** und konzentriert sich auf folgende Kernbereiche:
...
AyntraCore ist nach der **Hexagonalen Architektur (Ports & Adapters)** strukturiert.
...
4. **Anwendung starten**:
    - Via IntelliJ IDEA:
        - Rechtsklick auf `AyntraCoreApplication.java` → "Run".
    - Oder über Maven:
      ```bash
      mvn spring-boot:run
      ```
...
5. **Zugriffspunkte nach Start**:
    ...
    - **H2 Konsole**: `http://localhost:8080/h2-console`
        - Datenbank-URL: `jdbc:h2:mem:ayntracoredb`
...
```plaintext
src/main/java/com/ayntracore/
├── AyntraCoreApplication.java          # Hauptklasse
├── core/
│   ├── domain/
