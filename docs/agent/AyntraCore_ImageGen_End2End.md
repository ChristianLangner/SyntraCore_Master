...
# AyntraCore | Image Generation – End‑to‑End Blueprint (MD für Agenten)

**Version:** 1.0  
**Datum:** 10.02.2026  
**Ziel:** Einheitliche Spezifikation für Backend‑Datenstruktur, Admin‑UI (Sidebar) und Ausführungslogik inkl. Agent‑Ablaufplan.

---

## 1) Backend – Datenstruktur (Ebene 2)

### 1.1 Record: `ImageGenerationMetadata` (Java 21)
> **Robuste Default‑Quelle**, damit Modell‑/Sampler‑/Steps‑Änderungen zentral erfolgen.

```java
// Core Domain: reines Datenmodell (keine Framework-Abhängigkeiten)
...
**Integration in Persona‑Traits (JSONB)**
```json
{
  "themeColor": "#0EA5E9",
  "image": {
...
```
> **Multi‑Tenancy:** Persistenz pro Persona/Tenant im `traits`‑Feld (JSONB). Alle Abfragen **immer** mit `companyId` filtern.

### 1.2 Record: `ImageGenerationParams` (Java 21)
> **Runtime‑Parameter** pro Request, UI‑Overrides gegenüber Metadata.
...
### 1.3 Merge‑Helfer: `ImageCallArgs`
> Kombiniert Defaults aus `ImageGenerationMetadata` mit UI‑Parametern.
...
### 1.4 PersonaService – Seed‑Initialisierung (COMPANION)
```java
...
### 1.5 Adapter – CivitaiImageAdapter (Logging & Latenz)
```java
...
### 2.1 HTML (Slide‑Over in `admin.html`)
```html
<!-- Trigger (Preview-Bild) -->
...
### 2.2 JS (Seed‑Lock, Presets, Hot‑Persist)
```html
<script>
  let seedLocked = true;
...
## 3) 💬 Anweisung für Copilot (Copy & Paste)
...
## 4) Agent‑Ablaufplan (Execution Flow)

**Preflight**
1. `companyId` prüfen (Gate).  
2. Persona laden → `traits.image` (`seed`, `modelId`, `sampler`, `steps`, `cfgScale`, optional `seedLock`).

**Parameteraufbau**
3. UI‑Eingaben lesen: `prompt`, `cfgScale`, `model`, `seedLock`.  
4. **Seed‑Strategie**: Wenn `seedLock==true` → `traits.image.seed` verwenden; sonst neuen Seed würfeln und optional persistieren.  
5. **Merge**: `ImageCallArgs = merge(metadata, params)`.

**Aufruf & Ausgabe**
6. Adapter: `CivitaiImageAdapter.generate(ImageCallArgs)`; `[LATENCY]` loggen.  
7. Bild‑URL in Galerie/Preview einfügen.  
8. Traits/Audit aktualisieren (`lastModel`, `lastSeed`, `updatedAt`).

**UI‑Hints & Sicherheit**
9. UI‑Hints zurückgeben: `{ theming.primaryColor, streaming.enabled, transparency.showSourceCategories }`.  
10. Fehlerfälle: fehlende `companyId` blockieren; Werte validieren.

**Tests (DoD)**
- Seed‑Konsistenz nachweisbar (3 Läufe, gleiches Ergebnis bei Lock).  
- Presets ändern CFG/Model korrekt.  
- PATCH `/traits` schreibt `image`‑Block sauber (JSONB).  
- Logs vorhanden (`[IMAGE_CALL]`, `[LATENCY]`).
